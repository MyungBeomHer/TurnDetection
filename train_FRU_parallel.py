...
from FRU_Adapter import FRU_Adapter
...
CONFIG = {
    ...
    "num_epochs": 50, # overfitting starts to occur beyond 4 epochs
    "train_batch_size": 384, #384
    "eval_batch_size": 128,
    "warmup_ratio": 0.2,
    "weight_decay": 0.01,
...
if __name__ == "__main__":
    # 로컬에서 modal 없이 직접 호출
    # training_run은 @app.function 데코레이터 때문에 "함수핸들"이 됨 → 내부 코드를 복사해 새 함수를 하나 만드세요.
    # 아래는 안전한 방법: 동일 내용의 로컬 함수 추가 후 호출
    
    # def wrap_encoder_block(block, adapter, alpha=None):
    #     """
    #     병렬 부착: y = EncoderBlock(x) + alpha * Adapter(x)
    #     - EncoderBlock 호출 전에 x가 2D(T,D)이면 (1,T,D)로 승격 → orig_forward의 3D 요구를 만족
    #     - Adapter도 동일 shape(3D)에 적용
    #     - 출력 후 필요 시 squeeze(0)
    #     """
    ...
    #     #     block.register_parameter("adapter_alpha", alpha)

    #     def new_forward(self, hidden_states, *args, **kwargs):
    #         x_in = hidden_states
    #         squeezed = False
    #         if x_in.dim() == 2:          # (T, D) → (1, T, D)
    #             x_in = x_in.unsqueeze(0)
    #             squeezed = True
    #         elif x_in.dim() != 3:
    #             raise ValueError(f"Encoder expects 2D/3D, got {x_in.shape}")

    #         # 1) 원래 HF 블록 실행 (항상 3D로)
    #         outputs = orig_forward(x_in, *args, **kwargs)
    #         if isinstance(outputs, tuple):
    #             hs, *extras = outputs
    #         else:
    #             hs, extras = outputs, []

    #         # 2) 병렬 어댑터: 입력 x_in에 적용 (3D)
    #         delta = adapter(x_in)        # (B, T, D)

    #         # 3) 출력에서 residual 합산
    #         hs = hs + delta

    #         # 4) 상위가 2D를 기대했다면 다시 2D로
    #         if squeezed:
    #             hs = hs.squeeze(0)

    #         return (hs, *extras) if extras else hs

    #     block.forward = types.MethodType(new_forward, block)

    # def wrap_encoder_block(block, adapter):
    #     orig_forward = block.forward
    #     # 레이어별 스케일러 (선택, 학습 가능)
    #     def new_forward(self, hidden_states, *args, **kwargs):
    #         x_in = hidden_states                      # (B,T,D) 또는 (T,D)
    #         # 1) 원래 블록 실행
    ...
    #         # 2) 병렬 어댑터: 입력 x_in을 사용
    #         o_adapter = adapter(x_in)                     # (B,T,D) or (T,D)
    ...
    def training_run_local(run_number: str = "00"):
        # 여기부터는 training_run(...) 함수 본문을 그대로 복붙
        # 단, modal 관련 옵션(gpu, volumes)은 빼도 됩니다.
        # -----------------------------------------------
        # (원본 training_run 본문 시작)
        log_dependencies()
        now = datetime.now().strftime("%Y-%m-%d_%H:%M")
        CONFIG["run_name"] = f"v3-{now}_run{run_number}"
        log.info(f"Starting training run: {CONFIG['run_name']}")
        wandb_api_key = os.environ.get("WANDB_API_KEY")
        # 원한다면 여기서 API key 체크 주석 처리 가능
        wandb_run = wandb.init(project="speech-endpointing",
                               name=CONFIG["run_name"],
                               config=CONFIG)
        wandb_run.define_metric(name="exttest/*", step_metric="train/global_step")

        model = SmartTurnV3Model.from_pretrained(CONFIG["model_name"],
                                                 num_labels=1,
                                                 ignore_mismatched_sizes=True)
        feature_extractor = WhisperFeatureExtractor(chunk_length=8) # 8 sec
        ...
        datasets = prepare_datasets_ondemand(feature_extractor, CONFIG)
        training_args = TrainingArguments(
            output_dir=f"./output/{CONFIG['run_name']}",   # 로컬 경로로 변경
            per_device_train_batch_size=CONFIG["train_batch_size"],
            per_device_eval_batch_size=CONFIG["eval_batch_size"],
            num_train_epochs=CONFIG["num_epochs"],
            eval_strategy=IntervalStrategy.STEPS,
        ...
        feature_extractor.save_pretrained(final_save_path)
        trainer.save_model(final_save_path)
        # 이하 ONNX/양자화 부분은 그대로 둬도 되고, 처음엔 주석 처리해도 OK
        log.info(f"Training completed. Saved to: {final_save_path}")
        wandb.finish()
        # (원본 training_run 본문 끝)
        # -----------------------------------------------

    training_run_local("00")
