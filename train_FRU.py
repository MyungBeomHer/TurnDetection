...
class SmartTurnV3Model(WhisperPreTrainedModel):
    ...
    def forward(self, input_features, labels=None):
        ...
        #     input_features = blk(input_features) + self.fru_adapter[i](input_features)
        # encoder_outputs = input_features
...
if __name__ == "__main__":
    ...
    def wrap_encoder_block(block, adapter):
        ...
        def new_forward(self, hidden_states, *args, **kwargs):
                # HF 원래 블록 실행
            hs_in = adapter(hidden_states)      # 먼저 어댑터
            return orig_forward(hs_in + hidden_states, *args, **kwargs)
    ...
    def training_run_local(run_number: str = "00"):
        ...
        # 각 인코더 레이어 출력에 어댑터 추가
        d_model = model.config.d_model  # 1024 (medium 기준)
        adapters = nn.ModuleList([FRU_Adapter(d_model) for _ in range(model.config.encoder_layers)])
        model.encoder.adapters = adapters  # 모듈에 붙여두면 save/load도 함께 관리 가능
        for i, block in enumerate(model.encoder.layers):
            wrap_encoder_block(block, model.encoder.adapters[i])
